    ...
    internal class RefinedResultsHelper
    {
        ...
        internal static T? GetScalar<T>(IRelmContext relmContext, string query, Dictionary<string, object>? parameters = null, bool throwException = true)
        {
            return DatabaseWorkHelper.DoDatabaseWork<T>(relmContext, query,
                (cmd) =>
                {
                    return RunScalarCommand<T>(cmd, parameters: parameters);
                },
        ...
        {
            return await DatabaseWorkHelper.DoDatabaseWorkAsync<T>(relmContext, query,
        ...
        /// is returned.</returns>
        private async static Task<object> RunScalarCommandAsync<T>(MySqlCommand cmd, Dictionary<string, object>? parameters = null, CancellationToken cancellationToken = default)
        {
            if (parameters != null)
                cmd.Parameters.AddAllParameters(parameters);
        ...
        {
            return GetDataTableAsync(relmContext, query, parameters, throwException, CancellationToken.None)
                .GetAwaiter()
                .GetResult();
        }
